...
def load_data():
    ...
    import os
    path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "data", "leads_dataset.csv")
...
with tab4:
    ...
    if df.empty:
        ...
        with c2:
            ...
            industry_score = df.groupby("industry")["icp_score"].mean().reset_index() if "industry" in df.columns else pd.DataFrame(columns=["industry", "icp_score"])
        ...
        with c4:
            # Hot leads by title
            title_hot = df[df["status"]=="hot"].groupby("jobtitle").size().reset_index(name="hot_leads").rename(columns={"jobtitle": "jobtitle"})
            title_hot = title_hot.sort_values("hot_leads", ascending=False).head(8)
            fig4 = px.bar(title_hot, x="hot_leads", y="jobtitle",
                orientation="h",
